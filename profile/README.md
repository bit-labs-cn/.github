...
基于 Owl 核心框架构建的全功能后台管理系统，前后端一体化，提供完善的 RBAC 权限体系与管理界面。

`JWT 认证` `Casbin 授权` `用户/角色/菜单` `组织架构` `字典管理` `操作审计` `OAuth 登录` `Vue 3 前端`
...
│                          │                                    │
│   ┌──────────────────┐   │   ┌────────────────────────────┐   │
│   │    Owl Admin     │   │   │      Owl Admin (前端)      │   │
│   │  (权限管理框架)  │   │   │    (后台管理前端界面)      │   │
│   │  + 前端一体化    │   │   │    已整合到 owl-admin 仓库  │   │
│   └────────┬─────────┘   │   └─────────────┬──────────────┘   │
│            │              │                 │                  │
│            ▼              │                 ▼                  │
│   ┌──────────────────┐   │   ┌────────────────────────────┐   │
